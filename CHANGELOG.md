...
* Retain optimistic transaction databases with `Arc`, serialize native transaction access, and reject writes/resets that would invalidate live transaction views. See the README for the 1.0 migration contract.
* Make raw handle and snapshot-option contracts explicitly unsafe; retain direct compaction filters and comparators through database shutdown, serialize mutable filter/factory callbacks, and bind prefix-transform results to each input key's lifetime.
* Make options loading and its helper allocations exception-safe, reject incomplete native results, and exercise allocation failures and native error ownership in the memory checks.
